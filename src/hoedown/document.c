...
struct link_ref {
	...
	mulle_hoedown_buffer *link;
	mulle_hoedown_buffer *title;
...
typedef size_t
(*char_trigger)(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, uint8_t *data, size_t offset, size_t size);

static size_t char_emphasis(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, uint8_t *data, size_t offset, size_t size);
static size_t char_quote(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, uint8_t *data, size_t offset, size_t size);
static size_t char_linebreak(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, uint8_t *data, size_t offset, size_t size);
static size_t char_codespan(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, uint8_t *data, size_t offset, size_t size);
static size_t char_escape(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, uint8_t *data, size_t offset, size_t size);
static size_t char_entity(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, uint8_t *data, size_t offset, size_t size);
static size_t char_langle_tag(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, uint8_t *data, size_t offset, size_t size);
static size_t char_autolink_url(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, uint8_t *data, size_t offset, size_t size);
static size_t char_autolink_email(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, uint8_t *data, size_t offset, size_t size);
static size_t char_autolink_www(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, uint8_t *data, size_t offset, size_t size);
static size_t char_link(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, uint8_t *data, size_t offset, size_t size);
static size_t char_superscript(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, uint8_t *data, size_t offset, size_t size);
static size_t char_math(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, uint8_t *data, size_t offset, size_t size);
...
struct mulle_hoedown_document {
	mulle_hoedown_renderer md;
	mulle_hoedown_renderer_data data;

	struct link_ref *refs[REF_TABLE_SIZE];
	struct footnote_list footnotes_found;
	struct footnote_list footnotes_used;
	uint8_t active_char[256];
	mulle_hoedown_stack work_bufs[2];
	mulle_hoedown_extensions ext_flags;
	size_t max_nesting;
	int in_link_body;
};
...
static mulle_hoedown_buffer *
newbuf(mulle_hoedown_document *doc, int type)
{
	static const size_t buf_size[2] = {256, 64};
	mulle_hoedown_buffer *work = NULL;
	mulle_hoedown_stack *pool = &doc->work_bufs[type];
	...
	if (pool->size < pool->asize &&
		...
		work = mulle_hoedown_buffer_new(buf_size[type]);
		mulle_hoedown_stack_push(pool, work);
	}
...
static void
popbuf(mulle_hoedown_document *doc, int type)
{
	doc->work_bufs[type].size--;
}

static void
unscape_text(mulle_hoedown_buffer *ob, mulle_hoedown_buffer *src)
{
	size_t i = 0, org;
	while (i < src->size) {
		org = i;
		...
		if (i > org)
			mulle_hoedown_buffer_put(ob, src->data + org, i - org);

		if (i + 1 >= src->size)
			break;

		mulle_hoedown_buffer_putc(ob, src->data[i + 1]);
		i += 2;
	}
}
...
{
	struct link_ref *ref = mulle_hoedown_calloc(1, sizeof(struct link_ref));
...
free_link_refs(struct link_ref **references)
{
	...
	for (i = 0; i < REF_TABLE_SIZE; ++i) {
		...
		while (r) {
			next = r->next;
			mulle_hoedown_buffer_free(r->link);
			mulle_hoedown_buffer_free(r->title);
			free(r);
			r = next;
		}
	}
...
{
	struct footnote_ref *ref = mulle_hoedown_calloc(1, sizeof(struct footnote_ref));
...
{
	struct footnote_item *item = mulle_hoedown_calloc(1, sizeof(struct footnote_item));
	if (!item)
		return 0;
	item->ref = ref;
...
{
	mulle_hoedown_buffer_free(ref->contents);
	free(ref);
}
 ...
 */
static void
replace_spacing(mulle_hoedown_buffer *ob, const uint8_t *data, size_t size)
{
	size_t i = 0, mark;
	mulle_hoedown_buffer_grow(ob, size);
	while (1) {
		mark = i;
		while (i < size && data[i] != '\n') i++;
...
static size_t
tag_length(uint8_t *data, size_t size, mulle_hoedown_autolink_type *autolink)
{
	size_t i, j;
...
static void
parse_inline(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, uint8_t *data, size_t size)
{
	size_t i = 0, end = 0;
	mulle_hoedown_buffer work = { 0, 0, 0, 0, NULL, NULL, NULL };
	...
	while (i < size) {
		...
		if (doc->md.normal_text) {
			...
			mulle_hoedown_buffer_put(ob, data + i, end - i);
...
static size_t
parse_emph1(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, uint8_t *data, size_t size, uint8_t c)
{
	size_t i = 0, len;
	mulle_hoedown_buffer *work = 0;
	int r;
...
static size_t
parse_emph2(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, uint8_t *data, size_t size, uint8_t c)
{
	size_t i = 0, len;
	mulle_hoedown_buffer *work = 0;
	int r;
...
static size_t
parse_emph3(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, uint8_t *data, size_t size, uint8_t c)
{
	size_t i = 0, len;
	int r;
	...
	while (i < size) {
		...
		if (i + 2 < size && data[i + 1] == c && data[i + 2] == c && doc->md.triple_emphasis) {
			/* triple symbol found */
			mulle_hoedown_buffer *work = newbuf(doc, BUFFER_SPAN);
...
static size_t
parse_math(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, uint8_t *data, size_t offset, size_t size, const char *end, size_t delimsz, int displaymode)
{
	mulle_hoedown_buffer text = { NULL, 0, 0, 0, NULL, NULL, NULL };
	size_t i = delimsz;
...
static size_t
char_emphasis(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, uint8_t *data, size_t offset, size_t size)
{
	uint8_t c = data[0];
	size_t ret;
...
static size_t
char_linebreak(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, uint8_t *data, size_t offset, size_t size)
{
...
static size_t
char_codespan(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, uint8_t *data, size_t offset, size_t size)
{
	mulle_hoedown_buffer work = { NULL, 0, 0, 0, NULL, NULL, NULL };
...
static size_t
char_quote(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, uint8_t *data, size_t offset, size_t size)
{
...
static size_t
char_escape(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, uint8_t *data, size_t offset, size_t size)
{
	...
	if (size > 1) {
		...
		else mulle_hoedown_buffer_putc(ob, data[1]);
	} else if (size == 1) {
		mulle_hoedown_buffer_putc(ob, data[0]);
	}
...
static size_t
char_entity(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, uint8_t *data, size_t offset, size_t size)
{
	size_t end = 1;
	mulle_hoedown_buffer work = { 0, 0, 0, 0, NULL, NULL, NULL };
	...
	else mulle_hoedown_buffer_put(ob, data, end);
...
static size_t
char_langle_tag(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, uint8_t *data, size_t offset, size_t size)
{
	mulle_hoedown_buffer work = { NULL, 0, 0, 0, NULL, NULL, NULL };
	mulle_hoedown_autolink_type altype = HOEDOWN_AUTOLINK_NONE;
	...
	if (end > 2) {
		if (doc->md.autolink && altype != HOEDOWN_AUTOLINK_NONE) {
			mulle_hoedown_buffer *u_link = newbuf(doc, BUFFER_SPAN);
			work.data = data + 1;
			work.size = end - 2;
...
static size_t
char_autolink_www(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, uint8_t *data, size_t offset, size_t size)
{
	mulle_hoedown_buffer *link, *link_url, *link_text;
	...
	if ((link_len = mulle_hoedown_autolink__www(&rewind, link, data, offset, size, HOEDOWN_AUTOLINK_SHORT_DOMAINS)) > 0) {
		link_url = newbuf(doc, BUFFER_SPAN);
		HOEDOWN_BUFPUTSL(link_url, "http://");
		mulle_hoedown_buffer_put(link_url, link->data, link->size);
...
static size_t
char_autolink_email(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, uint8_t *data, size_t offset, size_t size)
{
	mulle_hoedown_buffer *link;
	size_t link_len, rewind;
	...
	if ((link_len = mulle_hoedown_autolink__email(&rewind, link, data, offset, size, 0)) > 0) {
		ob->size -= rewind;
...
static size_t
char_autolink_url(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, uint8_t *data, size_t offset, size_t size)
{
	mulle_hoedown_buffer *link;
	size_t link_len, rewind;
	...
	if ((link_len = mulle_hoedown_autolink__url(&rewind, link, data, offset, size, 0)) > 0) {
		ob->size -= rewind;
...
static size_t
char_link(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, uint8_t *data, size_t offset, size_t size)
{
	...
	mulle_hoedown_buffer *content = NULL;
	mulle_hoedown_buffer *link = NULL;
	mulle_hoedown_buffer *title = NULL;
	mulle_hoedown_buffer *u_link = NULL;
	...
	/* footnote link */
	if (is_footnote) {
		mulle_hoedown_buffer id = { NULL, 0, 0, 0, NULL, NULL, NULL };
	...
	if (i < size && data[i] == '(') {
		...
		if (link_e > link_b) {
			link = newbuf(doc, BUFFER_SPAN);
			mulle_hoedown_buffer_put(link, data + link_b, link_e - link_b);
		}

		if (title_e > title_b) {
			title = newbuf(doc, BUFFER_SPAN);
			mulle_hoedown_buffer_put(title, data + title_b, title_e - title_b);
		}
	...
	/* reference style link */
		...
		mulle_hoedown_buffer *id = newbuf(doc, BUFFER_SPAN);
		struct link_ref *lr;
		...
		if (link_b == link_e)
			...
			mulle_hoedown_buffer_put(id, data + link_b, link_e - link_b);
	...
	if (txt_e > 1) {
		...
		if (is_img) {
			mulle_hoedown_buffer_put(content, data + 1, txt_e - 1);
		} else {
...
static size_t
char_superscript(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, uint8_t *data, size_t offset, size_t size)
{
	size_t sup_start, sup_len;
	mulle_hoedown_buffer *sup;
...
static size_t
char_math(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, uint8_t *data, size_t offset, size_t size)
{
	/* double dollar */
	if (size > 1 && data[1] == '$')
...
static size_t
parse_codefence(uint8_t *data, size_t size, mulle_hoedown_buffer *lang, size_t *width, uint8_t *chr)
{
...
static int
is_atxheader(mulle_hoedown_document *doc, uint8_t *data, size_t size)
{
	if (data[0] != '#')
		return 0;
...
static void parse_block(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc,
			uint8_t *data, size_t size);


/* parse_blockquote • handles parsing of a blockquote fragment */
static size_t
parse_blockquote(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, uint8_t *data, size_t size)
{
	size_t beg, end = 0, pre, work_size = 0;
	uint8_t *work_data = 0;
	mulle_hoedown_buffer *out = 0;
	...
	while (beg < size) {
		...
		if (beg < end) { /* copy into the in-place working buffer */
			/* mulle_hoedown_buffer_put(work, data + beg, end - beg); */
			if (!work_data)
				work_data = data + beg;
...
static size_t
parse_htmlblock(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, uint8_t *data, size_t size, int do_render);

/* parse_blockquote • handles parsing of a regular paragraph */
static size_t
parse_paragraph(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, uint8_t *data, size_t size)
{
	mulle_hoedown_buffer work = { NULL, 0, 0, 0, NULL, NULL, NULL };
	size_t i = 0, end = 0;
	int level = 0;
	...
	if (!level) {
		mulle_hoedown_buffer *tmp = newbuf(doc, BUFFER_BLOCK);
		parse_inline(tmp, doc, work.data, work.size);
		if (doc->md.paragraph)
			doc->md.paragraph(ob, tmp, &doc->data);
		popbuf(doc, BUFFER_BLOCK);
	} else {
		mulle_hoedown_buffer *header_work;
...
static size_t
parse_fencedcode(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, uint8_t *data, size_t size)
{
	mulle_hoedown_buffer text = { 0, 0, 0, 0, NULL, NULL, NULL };
	mulle_hoedown_buffer lang = { 0, 0, 0, 0, NULL, NULL, NULL };
...
static size_t
parse_blockcode(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, uint8_t *data, size_t size)
{
	size_t beg, end, pre;
	mulle_hoedown_buffer *work = 0;
	...
	while (beg < size) {
		...
		if (beg < end) {
			...
			if (is_empty(data + beg, end - beg))
				mulle_hoedown_buffer_putc(work, '\n');
			else mulle_hoedown_buffer_put(work, data + beg, end - beg);
		}
		beg = end;
	}
...
static size_t
parse_listitem(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, uint8_t *data, size_t size, mulle_hoedown_list_flags *flags)
{
	mulle_hoedown_buffer *work = 0, *inter = 0;
	...
	while (beg < size) {
		...
		mulle_hoedown_buffer_put(work, data + beg + i, end - beg - i);
		beg = end;
	}
...
static size_t
parse_list(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, uint8_t *data, size_t size, mulle_hoedown_list_flags flags)
{
	mulle_hoedown_buffer *work = 0;
	size_t i = 0, j;
...
static size_t
parse_atxheader(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, uint8_t *data, size_t size)
{
	size_t level = 0;
	size_t i, end, skip;
...
static void
parse_footnote_def(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, unsigned int num, uint8_t *data, size_t size)
{
	mulle_hoedown_buffer *work = 0;
	work = newbuf(doc, BUFFER_SPAN);
...
static void
parse_footnote_list(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, struct footnote_list *footnotes)
{
	mulle_hoedown_buffer *work = 0;
...
parse_htmlblock(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, uint8_t *data, size_t size, int do_render)
{
	...
	if (i < size)
		curtag = mulle_hoedown_find_block_tag((char *)data + 1, (int)i - 1);
...
static void
parse_table_row(
	mulle_hoedown_buffer *ob,
	mulle_hoedown_document *doc,
	uint8_t *data,
	size_t size,
	size_t columns,
	mulle_hoedown_table_flags *col_data,
	mulle_hoedown_table_flags header_flag)
{
	size_t i = 0, col, len;
	mulle_hoedown_buffer *row_work = 0;
	...
	for (col = 0; col < columns && i < size; ++col) {
		...
		mulle_hoedown_buffer *cell_work;
	...
	for (; col < columns; ++col) {
		mulle_hoedown_buffer empty_cell = { 0, 0, 0, 0, NULL, NULL, NULL };
...
parse_table_header(
	...
	size_t size,
	size_t *columns,
	mulle_hoedown_table_flags **column_data)
{
	int pipes;
	...
	*columns = pipes + 1;
	*column_data = mulle_hoedown_calloc(*columns, sizeof(mulle_hoedown_table_flags));
	...
	size_t size)
{
	...
	mulle_hoedown_buffer *work = 0;
	mulle_hoedown_buffer *header_work = 0;
	mulle_hoedown_buffer *body_work = 0;

	size_t columns;
	mulle_hoedown_table_flags *col_data = NULL;
...
/* parse_block • parsing of one block, returning next uint8_t to parse */
static void
parse_block(mulle_hoedown_buffer *ob, mulle_hoedown_document *doc, uint8_t *data, size_t size)
{
	size_t beg, end, i;
	uint8_t *txt_data;
	beg = 0;
...
{
	size_t i = 0;
	mulle_hoedown_buffer *contents = 0;
	size_t ind = 0;
	int in_empty = 0;
	size_t start = 0;
	...
	contents = mulle_hoedown_buffer_new(64);
	...
	while (i < end) {
		...
		if (ind == 0) {
			...
			mulle_hoedown_buffer_putc(contents, '\n');
		}

		in_empty = 0;

		/* adding the line into the content buffer */
		mulle_hoedown_buffer_put(contents, data + start + ind, i - start - ind);
		/* add carriage return */
		if (i < end) {
			mulle_hoedown_buffer_putc(contents, '\n');
			if (i < end && (data[i] == '\n' || data[i] == '\r')) {
...
/*	int n; */
	...
	if (refs) {
		...
		ref->link = mulle_hoedown_buffer_new(link_end - link_offset);
		mulle_hoedown_buffer_put(ref->link, data + link_offset, link_end - link_offset);

		if (title_end > title_offset) {
			ref->title = mulle_hoedown_buffer_new(title_end - title_offset);
			mulle_hoedown_buffer_put(ref->title, data + title_offset, title_end - title_offset);
		}
	}
...
static void expand_tabs(mulle_hoedown_buffer *ob, const uint8_t *line, size_t size)
{
	...
	while (i < size) {
		...
		if (i > org)
			mulle_hoedown_buffer_put(ob, line + org, i - org);

		if (i >= size)
			break;

		do {
			mulle_hoedown_buffer_putc(ob, ' '); tab++;
		} while (tab % 4);
...
mulle_hoedown_document *
mulle_hoedown_document_new(
	const mulle_hoedown_renderer *renderer,
	mulle_hoedown_extensions extensions,
	size_t max_nesting)
{
	mulle_hoedown_document *doc = NULL;

	assert(max_nesting > 0 && renderer);

	doc = mulle_hoedown_malloc(sizeof(mulle_hoedown_document));
	memcpy(&doc->md, renderer, sizeof(mulle_hoedown_renderer));

	doc->data.opaque = renderer->opaque;

	mulle_hoedown_stack_init(&doc->work_bufs[BUFFER_BLOCK], 4);
	mulle_hoedown_stack_init(&doc->work_bufs[BUFFER_SPAN], 8);
...
void
mulle_hoedown_document_render(mulle_hoedown_document *doc, mulle_hoedown_buffer *ob, const uint8_t *data, size_t size)
{
	static const uint8_t UTF8_BOM[] = {0xEF, 0xBB, 0xBF};

	mulle_hoedown_buffer *text;
	size_t beg, end;

	int footnotes_enabled;

	text = mulle_hoedown_buffer_new(64);

	/* Preallocate enough space for our buffer to avoid expanding while copying */
	mulle_hoedown_buffer_grow(text, size);
		...
		if (footnotes_enabled && is_footnote(data, beg, size, &end, &doc->footnotes_found))
			...
			while (end < size && (data[end] == '\n' || data[end] == '\r')) {
				...
				if (data[end] == '\n' || (end + 1 < size && data[end + 1] != '\n'))
					mulle_hoedown_buffer_putc(text, '\n');
				end++;
			}
	...
	mulle_hoedown_buffer_grow(ob, text->size + (text->size >> 1));
	...
	/* clean-up */
	mulle_hoedown_buffer_free(text);
...
void
mulle_hoedown_document_render_inline(mulle_hoedown_document *doc, mulle_hoedown_buffer *ob, const uint8_t *data, size_t size)
{
	size_t i = 0, mark;
	mulle_hoedown_buffer *text = mulle_hoedown_buffer_new(64);
...
void
mulle_hoedown_document_free(mulle_hoedown_document *doc)
{
	size_t i;

	for (i = 0; i < (size_t)doc->work_bufs[BUFFER_SPAN].asize; ++i)
		mulle_hoedown_buffer_free(doc->work_bufs[BUFFER_SPAN].item[i]);

	for (i = 0; i < (size_t)doc->work_bufs[BUFFER_BLOCK].asize; ++i)
		mulle_hoedown_buffer_free(doc->work_bufs[BUFFER_BLOCK].item[i]);

	mulle_hoedown_stack_uninit(&doc->work_bufs[BUFFER_SPAN]);
	mulle_hoedown_stack_uninit(&doc->work_bufs[BUFFER_BLOCK]);
